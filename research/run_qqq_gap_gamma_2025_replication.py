#!/usr/bin/env python3
"""Frozen Investing OS experiment: QQQ 2025 gap-conditioned gamma replication v1.0.

Rules are frozen in research/qqq_gap_gamma_2025_replication_v1.md and must not be
...
SRC = Path(os.environ.get("QQQ_2025_OPTIONS_PARQUET", "/tmp/qqq_options_2025.parquet"))
...
def main():
    ...
    if len(panel) < 70:
        raise RuntimeError(f"Too few qualifying QQQ 2025 gap sessions for frozen replication: {len(panel)}")

    corr = correlation_table(panel)
    split = median_split(panel)
    preds, wf = walk_forward(panel)

    prefix = "qqq_gap_gamma_2025_replication_v1"
    panel.to_csv(OUT / f"{prefix}_session_panel.csv", index=False)
    corr.to_csv(OUT / f"{prefix}_correlations.csv", index=False)
    preds.to_csv(OUT / f"{prefix}_walkforward_predictions.csv", index=False)
    wf.to_csv(OUT / f"{prefix}_walkforward_summary.csv", index=False)
    ...
    lines = []
    lines.append("# QQQ 2025 Gap-Conditioned Gamma Time-Shift Replication v1.0 — Frozen Results\n")
    ...
    lines.append("- Time independence: QQQ calendar 2025; no 2026 QQQ discovery observations used.")
    lines.append("- Source independence: 2025 historical archive differs from the 2026 daily-chain source.")
    ...
    report = "\n".join(lines)
    (OUT / f"{prefix}_results.md").write_text(report)
    print(report)

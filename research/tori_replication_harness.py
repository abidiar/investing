...
TouchClusterMode = Literal['earliest', 'min_error']
...
class ReplicationProfile:
    ...
    touch_cluster_mode: TouchClusterMode = 'earliest'
...
def reconstruct_htf(m30: pd.DataFrame) -> pd.DataFrame:
    ...
    for d, g in x.groupby(x['time_et'].dt.date, sort=True):
        ...
        for label, sh, sm, eh, em in [('AM',9,30,13,0),('PM',13,0,16,0)]:
            ...
            rows.append({
                'session_date': d, 'half': label, 'time': end_et.tz_convert('UTC'),
                'open': float(z.iloc[0]['open']), 'high': float(z['high'].max()),
                'low': float(z['low'].min()), 'close': float(z.iloc[-1]['close']),
                'volume': float(z['volume'].fillna(0).sum()), 'm30_count': int(len(z)),
            })
...
def add_atr(htf: pd.DataFrame, mode: ATRMode='wilder', period: int=14) -> pd.DataFrame:
    h = htf.copy(); prev = h['close'].shift(1)
...
def pivot_high_indices(h: pd.DataFrame) -> list[int]:
    highs=h['high'].to_numpy(float); out=[]
...
def _atr_ref(h: pd.DataFrame, mode: ATRRefMode, idx: int, breakout_i: int, touches: Iterable[int]) -> float:
    if mode=='local': return float(h.at[idx,'atr'])
    if mode=='breakout': return float(h.at[breakout_i,'atr'])
...
def _fit_pair(i: int,j: int,h: pd.DataFrame)->tuple[float,float]:
    yi=float(h.at[i,'high']); yj=float(h.at[j,'high']); slope=(yj-yi)/(j-i)
...
def _fit_ols(idxs: list[int],h: pd.DataFrame)->tuple[float,float]:
    ...
    slope,intercept=np.polyfit(x,y,1); return float(slope),float(intercept)


def _touches_for_line(h,pivots,slope,intercept,breakout_i,profile):
    prelim=[]; errs=[]
    for p in pivots:
    ...
    for p,e in zip(prelim,errs):
        ...
        elif profile.touch_cluster_mode=='min_error' and e<kept_err[-1]:
            kept[-1]=p; kept_err[-1]=e
    return kept,kept_err


def _session_span_ok(h,first,third):
    return int(h.at[third,'session_ord'])-int(h.at[first,'session_ord'])>=MIN_FIRST_THIRD_SESSIONS


def _candidate_at_breakout(h,confirmed_pivots,breakout_i,profile):
        ...
        for b_pos in range(a_pos+1,len(confirmed_pivots)):
            ...
            for q in range(first,breakout_i):
                ...
                if float(h.at[q,'close'])>slope*q+intercept+INTEGRITY_CLOSE_ATR*ref+1e-12:
                    ok=False; break
            if not ok: continue
            action=slope*breakout_i+intercept; close=float(h.at[breakout_i,'close'])
            if close<=action+BREAKOUT_CLOSE_ATR*atr_b+1e-12: continue
            out.append(LineCandidate(
                breakout_i=breakout_i,breakout_time=h.at[breakout_i,'time'],first_touch_i=first,third_touch_i=third,
                touch_indices=tuple(touches),anchor_indices=(i,j),slope=slope,intercept=intercept,
                action_line=float(action),atr_breakout=atr_b,extension_atr=float((close-action)/atr_b),
                mean_touch_error_atr=float(np.mean(errs)),history_bars=breakout_i-first))
    return out


def raw_candidates(htf,profile):
    h=add_atr(htf,profile.atr_mode); piv=pivot_high_indices(h); allc=[]
    for b in range(len(h)):
        confirmed=[p for p in piv if p+PIVOT_SIDE<=b]
        if len(confirmed)>=MIN_TOUCHES: allc.extend(_candidate_at_breakout(h,confirmed,b,profile))
    return h,allc


def _canonical_family(cands):
    return sorted(cands,key=lambda c:(-c.touches,-c.history_bars,c.mean_touch_error_atr,c.breakout_i))[0]


def dedupe_families(cands):
    if not cands: return []
    cands=sorted(cands,key=lambda c:(c.breakout_i,c.action_line)); n=len(cands); parent=list(range(n))
    def find(x):
        while parent[x]!=x:
    ...
    for a in range(n):
        for b in range(a+1,n):
            ...
            if abs(ca.action_line-cb.action_line)<=FAMILY_LINE_TOL_ATR*atr+1e-12: union(a,b)
    groups={}
...
def apply_regime_lockout(families):
    if not families: return []
    accepted=[]; last_break=None
    for c in sorted(families,key=lambda x:x.breakout_i):
        ...
        old_anchors=sum(1 for p in c.anchor_indices if p<=last_break)
        if new_touches>=2 and old_anchors<=1:
            accepted.append(c); last_break=c.breakout_i
    return accepted


def map_entries(events,h,m30):
    x=_normalize_m30(m30); out=[]
    for c in events:
        z=x[x['time']>=c.breakout_time]; row=z.iloc[0] if not z.empty else None
        out.append(Event(
            breakout_i=c.breakout_i,breakout_time=c.breakout_time,entry_time=None if row is None else row['time'],
            entry_price=None if row is None else float(row['open']),action_line=c.action_line,atr_breakout=c.atr_breakout,
            extension_atr=c.extension_atr,touches=c.touches,first_touch_i=c.first_touch_i,third_touch_i=c.third_touch_i,
            touch_indices=c.touch_indices,anchor_indices=c.anchor_indices,slope=c.slope,mean_touch_error_atr=c.mean_touch_error_atr))
    return out


def detect(m30,profile):
    base=reconstruct_htf(m30); h,c=raw_candidates(base,profile); fam=dedupe_families(c); locked=apply_regime_lockout(fam)
    return h,map_entries(locked,h,m30)


def tori_early(events):
    return [e for e in events if e.extension_atr<=TORI_EARLY_MAX_EXTENSION_ATR+1e-12]


def coverage_audit(m30):
    h=reconstruct_htf(m30); p=h.pivot_table(index='session_date',columns='half',values='m30_count',aggfunc='first')
    p['normal_complete']=(p.get('AM')==7)&(p.get('PM')==6); return p.reset_index()
            ...
            for touch_ref in ['local','breakout']:
                for integrity_ref in ['local','breakout']:
                    for slope_ref in ['breakout','mean_touches']:
                        for cluster in ['earliest','min_error']:
                            k+=1
                            profiles.append(ReplicationProfile(
                                name=f'P{k:03d}_{atr}_{line}_T{touch_ref}_I{integrity_ref}_S{slope_ref}_C{cluster}',
                                atr_mode=atr,line_mode=line,touch_atr_ref=touch_ref,integrity_atr_ref=integrity_ref,
                                slope_atr_ref=slope_ref,touch_cluster_mode=cluster))
    return profiles
